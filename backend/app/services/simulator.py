...
class StadiumSimulator:
    ...
    def get_state(self) -> dict[str, Any]:
        ...
        gates = {
            gid: round(min(99, max(5, phase * 100 + rng.gauss(0, 8))), 1)
            for gid in GATE_IDS
        }
        zones = {
            zid: round(min(99, max(5, phase * 90 + rng.gauss(0, 10))), 1)
            for zid in ZONE_IDS
        }
        concessions = {
            cid: round(max(1, phase * 12 + rng.gauss(0, 2)), 1)
...
class DeterministicMode:
    ...
    def _state_for_phase(self, phase: float, event: str, tick: int) -> dict[str, Any]:
        ...
        gates = {
            gid: round(min(99, max(5, phase * 100 + rng.gauss(0, 6))), 1)
            for gid in GATE_IDS
        }
        zones = {
            zid: round(min(99, max(5, phase * 90 + rng.gauss(0, 8))), 1)
            for zid in ZONE_IDS
        }
        concessions = {
            cid: round(max(1, phase * 12 + rng.gauss(0, 1.5)), 1)
            for cid in CONCESSION_IDS
        }
        return {
            "timestamp": time.time(),
